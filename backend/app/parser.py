...
class ResultParser:
    ...
    def parse_html(html_content):
        ...
        # Get metadata
        metadata = {
            'title': soup.find('h3', {'align': 'center'}).text.strip() if soup.find('h3', {'align': 'center'}) else '',
            'header_image': soup.find('img', {'src': 'lms-head.png'}).get('src') if soup.find('img', {'src': 'lms-head.png'}) else ''
        }
